...
# Marsscapes imports
from marsscapes.helpers.csHelpers import *
...
if CSUPPORT:
    try:
        from marsscapes.evaluation import addToConfusionMatrix
    except:
        CSUPPORT = False
...
#   - environment variable MARSSCAPES_RESULTS
#   - environment variable MARSSCAPES_DATASET/results
#   - ../../results/"
#
...
# <mars>_123456_123456*.png
# for a ground truth filename
# <mars>_123456_123456_gtFine_labelIds.png
def getPrediction( args, groundTruthFile ):
    ...
    if not args.predictionPath:
        rootPath = None
        if 'MARSSCAPES_RESULTS' in os.environ:
            rootPath = os.environ['MARSSCAPES_RESULTS']
        elif 'MARSSCAPES_DATASET' in os.environ:
            rootPath = os.path.join( os.environ['MARSSCAPES_DATASET'] , "results" )
        else:
    ...
    filePattern = "{}_{}_{}*.png".format( csFile.mars , csFile.sequenceNb , csFile.frameNb )
...
# Where to look for Marsscapes
if 'MARSSCAPES_DATASET' in os.environ:
    args.marsscapesPath = os.environ['MARSSCAPES_DATASET']
else:
    args.marsscapesPath = os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..')

if 'MARSSCAPES_EXPORT_DIR' in os.environ:
    export_dir = os.environ['MARSSCAPES_EXPORT_DIR']
    if not os.path.isdir(export_dir):
        raise ValueError("MARSSCAPES_EXPORT_DIR {} is not a directory".format(export_dir))
    args.exportFile = "{}/resultPixelLevelSemanticLabeling.json".format(export_dir)
else:
    args.exportFile = os.path.join(args.marsscapesPath, "evaluationResults", "resultPixelLevelSemanticLabeling.json")
# Parameters that should be modified by user
args.groundTruthSearch  = os.path.join( args.marsscapesPath , "gtFine" , "val" , "*", "*_gtFine_labelIds.png" )
